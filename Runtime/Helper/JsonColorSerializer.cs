...
{
    /// <summary>
    /// Json.NET converter for Unity's <see cref="Color"/>.
    /// Shared between packages that persist settings/definitions.
    /// </summary>
    public sealed class SerializerJsonColorConverter : JsonConverter<Color>
    {
        ...
        public static T DeserializeColor<T>(object value, T defaultValue)
        {
            if (value is string colorString)
            {
                try
                {
                    var color = JsonConvert.DeserializeObject<Color>(colorString, new SerializerJsonColorConverter());
